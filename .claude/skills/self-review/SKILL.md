...
レビュアーには、指摘ごとに次のどれに当たるかを明示させる。severity での足切りはレビュアーにさせず、確信の持てないものも含めて全部出させる。

- `carried-over`: 前ラウンドの指摘が未解消、または見送り理由が妥当でない
- `regression`: 前ラウンドの修正が持ち込んだ問題
- `new`: 上のどちらでもなく、今回初めて出た指摘

受け取った指摘は依頼元がこう振り分ける:

- `carried-over` と `regression` は severity を問わず 4 に戻す
- `new` は must-fix だけを 4 に戻す。should-fix と nit は記録に残し、ループには入れず終了時に報告する

`new` の should-fix でループを延ばさないのは、ラウンド1で全観点を並行レビューした結果を信頼するため。懐疑的に見ろと指示されたレビュアーは新鮮な目で見るたびに何かしら出すので、これを無条件に拾うと終了条件が閉じない。

### 6. 終了判定とユーザーへの報告

- 4 に戻す指摘がなくなったら終了。レビュアーが approve を返した場合と、request-changes だが残った指摘が 5 のゲートで見送りに振り分けられたものだけの場合の両方が該当する

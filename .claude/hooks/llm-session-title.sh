...
(
  title=$(TITLE_HOOK_ACTIVE=1 "${limit[@]}" claude -p \
    ...
    "以下はコーディングセッションの最初のプロンプトです。このセッションを一覧から見分けるための短いタイトルを1つだけ出力してください。タイトルは必ず日本語で20文字以内。記号や引用符は使わず、タイトル以外は一切出力しないでください。参照先を読んだかどうかや、どう判断したかは書かず、タイトルだけを出力してください。20文字に収まらないときは情報を削って収めてください。

プロンプトの文面だけでは何をするのか分からず、issue 番号 (#42 など) や URL が作業対象を指しているときは、gh issue view や WebFetch でその参照先を読んでください。
読めたなら、番号ではなく参照先の中身からタイトルを付けてください。読めなかったなら、そのことはタイトルに書かず、プロンプトの文面 (番号を含む) からタイトルを付けてください。

やってよいのは読み取りだけです。ファイルの作成・編集・削除、git の操作、コメントの投稿など、状態を変える操作はしないでください。

プロンプト:
$prompt" 2> "$cache.err" | grep -m1 '[^[:space:]]')
  # タイトルではなく「参照先が読めなかったので文面から付ける」のような作業の説明が1行目に来ることがある。
  # 実測では説明は19文字から130文字に散り、正当なタイトルも長いファイル名を含むと32文字まで伸びる。
  # 長さで両者を分けきることはできない。30文字を超えたら捨てて次回に回す
  if [ -n "$title" ] && [ "$(printf '%s' "$title" | jq -Rs 'length')" -le 30 ] &&
